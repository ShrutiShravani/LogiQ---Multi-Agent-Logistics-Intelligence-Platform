...
def test_document_agent_process_success():
    # 1. SETUP: Create a "Fake" LLM response
    mock_llm = MagicMock()
    
    # This is what we want the LLM to 'pretend' to return
    fake_llm_json = {
    ...
    # Configure the mock to return an object with a .content attribute (like LangChain)
    mock_response = MagicMock()
    mock_response.content = json.dumps(fake_llm_json)
 
    mock_llm.invoke.return_value = mock_response

    # 2. ACT: Run the agent with the mock LLM
    agent = DocumentAgent(llm_client=mock_llm)
    result = agent.process("This is some dummy waybill text")

    # 3. ASSERT: Check if the agent correctly transformed the JSON into a Pydantic Model
    assert isinstance(result, ShipmentModel)
    assert result.shipment_id == "WAYBILL-999"
    assert result.total_weight_kg == 5.5

    
    # Verify the LLM was actually called once
